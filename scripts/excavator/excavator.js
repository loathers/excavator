...
var ZONE_PATTERNS = /* @__PURE__ */ new Map([["Conspiracy Island", /The swarm of mayflies buzzes off into the spooky distance and returns a few minutes later, struggling under the weight of a mysterious coin/], ["Dinseylandfill", /The swarm of mayflies buzzes off into the distance and returns a few moments later carrying a rolled up FunFunds/], ["Twitch", /The swarm of mayflies buzzes off into the greasy distance and returns a few minutes later, struggling under the weight of a shiny coin/], ["Spring Break Beach", /The swarm of mayflies buzzes off into the greasy distance and returns a few minutes later, struggling under the weight of a damp bill/], ["The Glaciest", /The swarm of mayflies buzzes off into the frigid distance and returns a short while later carrying a rolled up Wal-Mart gift certificate/], ["Dreadsylvania", /The swarm of mayflies buzzes off into the dreary distance and returns a few minutes later, struggling under the weight of a coin/], ["Degrassi Knoll", /The swarm of mayflies buzzes off in the direction of one of the myriad workbenches littering the Knoll/]]), LOCATION_PATTERNS = /* @__PURE__ */ new Map([[import_kolmafia19.Location.get("Hobopolis Town Square"), /The swarm of mayflies buzzes off into the distance and returns a few minutes later, struggling under the weight of a coin/], [import_kolmafia19.Location.get("The Hole in the Sky"), /mistakes the swarm of flies for a different (and apparently very frightening) constellation, and runs off so fast it leaves some little bits of itself behind/], [import_kolmafia19.Location.get("The Sleazy Back Alley"), /The swarm flies into a dumpster, buzzes around for a bit, and flies back out looking slightly larger/], [import_kolmafia19.Location.get("The Slime Tube"), /The swarm of mayflies plunges into the mass of Slime and emerges a few moments later, coated with goo and carrying a little glob of something/], [import_kolmafia19.Location.get("South of The Border"), /After the fight is over, one of those annoying kids runs up and tries to sell you some gum/]]), SWARM_PATTERN = /mayflies(\d+)\.gif/, ITEM_PATTERN = /The swarm of mayflies buzzes around the ground, helping you find stuff/, MEAT_PATTERN = /The swarm of mayflies draws your attention to more Meat than you would've otherwise found/, SUMMON_MAYFLY_SWARM = {
  ...
  hooks: {
    COMBAT_ROUND: function(_, page) {
      ...
      if (!isEquippedAtEndOfCombat(import_kolmafia19.Item.get("mayfly bait necklace"))) return null;
      var specialPattern = (_ZONE_PATTERNS$get = ZONE_PATTERNS.get((0, import_kolmafia19.myLocation)().zone)) !== null && _ZONE_PATTERNS$get !== void 0 ? _ZONE_PATTERNS$get : LOCATION_PATTERNS.get((0, import_kolmafia19.myLocation)());
      if (!specialPattern) return null;
      var swarmResult = page.match(SWARM_PATTERN);
      if (!swarmResult) return null;
      var swarmSize = Number(swarmResult[1]), area = ZONE_PATTERNS.has((0, import_kolmafia19.myLocation)().zone) ? (0, import_kolmafia19.myLocation)().zone : "".concat((0, import_kolmafia19.myLocation)()), patterns = {
...
function spadeRiftlet(encounter, page) {
  var riftlet = import_kolmafia20.Familiar.get("Temporal Riftlet");
  return (0, import_kolmafia20.currentRound)() !== 0 || (0, import_kolmafia20.myFamiliar)() !== riftlet ? null : {
    weight: (0, import_kolmafia20.familiarWeight)(riftlet) + (0, import_kolmafia20.weightAdjustment)() + riftlet.soupWeight + (riftlet.feasted ? 10 : 0),
...
var createPropertyGetter = function(transform) {
  return function(property, default_) {
    var value = (0, import_kolmafia22.getProperty)(property);
...
var getString = createPropertyGetter(function(value) {
  ...
  return Number(value);
}), getBounty = createMafiaClassPropertyGetter(import_kolmafia22.Bounty, import_kolmafia22.toBounty), getClass = createMafiaClassPropertyGetter(import_kolmafia22.Class, import_kolmafia22.toClass), getCoinmaster = createMafiaClassPropertyGetter(import_kolmafia22.Coinmaster, import_kolmafia22.toCoinmaster), getEffect = createMafiaClassPropertyGetter(import_kolmafia22.Effect, import_kolmafia22.toEffect), getElement = createMafiaClassPropertyGetter(import_kolmafia22.Element, import_kolmafia22.toElement), getFamiliar = createMafiaClassPropertyGetter(import_kolmafia22.Familiar, import_kolmafia22.toFamiliar, familiarNumericProperties), getItem = createMafiaClassPropertyGetter(import_kolmafia22.Item, import_kolmafia22.toItem, itemNumericProperties), getLocation = createMafiaClassPropertyGetter(import_kolmafia22.Location, import_kolmafia22.toLocation), getMonster = createMafiaClassPropertyGetter(import_kolmafia22.Monster, import_kolmafia22.toMonster, monsterNumericProperties), getPhylum = createMafiaClassPropertyGetter(import_kolmafia22.Phylum, import_kolmafia22.toPhylum), getServant = createMafiaClassPropertyGetter(import_kolmafia22.Servant, import_kolmafia22.toServant), getSkill = createMafiaClassPropertyGetter(import_kolmafia22.Skill, import_kolmafia22.toSkill), getSlot = createMafiaClassPropertyGetter(import_kolmafia22.Slot, import_kolmafia22.toSlot), getStat = createMafiaClassPropertyGetter(import_kolmafia22.Stat, import_kolmafia22.toStat), getThrall = createMafiaClassPropertyGetter(import_kolmafia22.Thrall, import_kolmafia22.toThrall);
function get(property, _default) {
  ...
  return _default instanceof import_kolmafia22.Location ? getLocation(property, _default) : _default instanceof import_kolmafia22.Monster ? getMonster(property, _default) : _default instanceof import_kolmafia22.Familiar ? getFamiliar(property, _default) : _default instanceof import_kolmafia22.Stat ? getStat(property, _default) : _default instanceof import_kolmafia22.Phylum ? getPhylum(property, _default) : _default instanceof import_kolmafia22.Item ? getItem(property, _default) : typeof _default == "boolean" ? value === "true" ? !0 : value === "false" ? !1 : _default : typeof _default == "number" ? value === "" ? _default : parseInt(value) : value === "" ? _default === void 0 ? "" : _default : value;
}
function _set(property, value) {
  var stringValue = value === null ? "" : value.toString();
  return (0, import_kolmafia22.setProperty)(property, stringValue), value;
}
  ...
  return _createClass(PropertiesManager2, [{
    ...
    value: function(propertiesToSet) {
      for (var _i2 = 0, _Object$entries2 = Object.entries(propertiesToSet); _i2 < _Object$entries2.length; _i2++) {
        ...
        propertyName in this.properties || (this.properties[propertyName] = (0, import_kolmafia22.propertyExists)(propertyName) ? get(propertyName) : PropertiesManager2.EMPTY_PREFERENCE), _set(propertyName, propertyValue);
      }
    }
    /**
    ...
    value: function() {
      ...
      for (var _i3 = 0, _properties = properties; _i3 < _properties.length; _i3++) {
        ...
        if (property in this.properties) {
          ...
          value === PropertiesManager2.EMPTY_PREFERENCE ? (0, import_kolmafia22.removeProperty)(property) : _set(property, value);
        }
      }
    }
    /**
...
var concatTemplateString = function(literals) {
  ...
  match ? (0, import_kolmafia23.print)("".concat(match[0], "; if you're certain that this ").concat(Type.name, " exists and is spelled correctly, please update KoLMafia"), "red") : (0, import_kolmafia23.print)(message);
...
}, $bounty = createSingleConstant(import_kolmafia23.Bounty, import_kolmafia23.toBounty), $bounties = createPluralConstant(import_kolmafia23.Bounty), $class = createSingleConstant(import_kolmafia23.Class, import_kolmafia23.toClass), $classes = createPluralConstant(import_kolmafia23.Class), $coinmaster = createSingleConstant(import_kolmafia23.Coinmaster, import_kolmafia23.toCoinmaster), $coinmasters = createPluralConstant(import_kolmafia23.Coinmaster), $effect = createSingleConstant(import_kolmafia23.Effect, import_kolmafia23.toEffect), $effects = createPluralConstant(import_kolmafia23.Effect), $element = createSingleConstant(import_kolmafia23.Element, import_kolmafia23.toElement), $elements = createPluralConstant(import_kolmafia23.Element), $familiar = createSingleConstant(import_kolmafia23.Familiar, import_kolmafia23.toFamiliar), $familiars = createPluralConstant(import_kolmafia23.Familiar), $item = createSingleConstant(import_kolmafia23.Item, import_kolmafia23.toItem), $items = createPluralConstant(import_kolmafia23.Item), $location = createSingleConstant(import_kolmafia23.Location, import_kolmafia23.toLocation), $locations = createPluralConstant(import_kolmafia23.Location), $modifier = createSingleConstant(import_kolmafia23.Modifier, import_kolmafia23.toModifier), $modifiers = createPluralConstant(import_kolmafia23.Modifier), $monster = createSingleConstant(import_kolmafia23.Monster, import_kolmafia23.toMonster), $monsters = createPluralConstant(import_kolmafia23.Monster), $path = createSingleConstant(import_kolmafia23.Path, import_kolmafia23.toPath), $paths = createPluralConstant(import_kolmafia23.Path), $phylum = createSingleConstant(import_kolmafia23.Phylum, import_kolmafia23.toPhylum), $phyla = createPluralConstant(import_kolmafia23.Phylum), $servant = createSingleConstant(import_kolmafia23.Servant, import_kolmafia23.toServant), $servants = createPluralConstant(import_kolmafia23.Servant), $skill = createSingleConstant(import_kolmafia23.Skill, import_kolmafia23.toSkill), $skills = createPluralConstant(import_kolmafia23.Skill), $slot = createSingleConstant(import_kolmafia23.Slot, import_kolmafia23.toSlot), $slots = createPluralConstant(import_kolmafia23.Slot), $stat = createSingleConstant(import_kolmafia23.Stat, import_kolmafia23.toStat), $stats = createPluralConstant(import_kolmafia23.Stat), $thrall = createSingleConstant(import_kolmafia23.Thrall, import_kolmafia23.toThrall), $thralls = createPluralConstant(import_kolmafia23.Thrall);
...
var byStat = makeByXFunction(function() {
  return (0, import_kolmafia24.myPrimestat)().toString();
}), byClass = makeByXFunction(function() {
  return (0, import_kolmafia24.myClass)().toString();
});
function extractItems(text) {
  return new Map(Object.entries((0, import_kolmafia24.extractItems)(text)).map(function(_ref13) {
    var _ref14 = _slicedToArray8(_ref13, 2), itemName = _ref14[0], quantity = _ref14[1];
    return [import_kolmafia24.Item.get(itemName), quantity];
  }));
}
function makeScalerCalcFunction(cache, pattern) {
  return function(monster) {
    var _find, _pattern$exec$slice, _pattern$exec, current = cache.get(monster);
    if (current !== void 0) return (0, import_kolmafia24.monsterEval)(current);
    ...
    return cache.set(monster, result), (0, import_kolmafia24.monsterEval)(result);
  };
}
...
var makeBulkFunction = function(action) {
  return function(items) {
    ...
    return (0, import_kolmafia24.batchClose)();
  };
}, bulkAutosell = makeBulkFunction(import_kolmafia24.autosell), bulkPutCloset = makeBulkFunction(import_kolmafia24.putCloset), bulkPutDisplay = makeBulkFunction(import_kolmafia24.putDisplay), bulkPutStash = makeBulkFunction(import_kolmafia24.putStash), bulkTakeCloset = makeBulkFunction(import_kolmafia24.takeCloset), bulkTakeDisplay = makeBulkFunction(import_kolmafia24.takeDisplay), bulkTakeShop = makeBulkFunction(import_kolmafia24.takeShop), bulkTakeStash = makeBulkFunction(import_kolmafia24.takeStash), bulkTakeStorage = makeBulkFunction(import_kolmafia24.takeStorage);
...
init_kolmafia_polyfill();
var import_html_entities = __toESM(require_lib(), 1), import_kolmafia26 = require("kolmafia");
...
function fetchUrl(path) {
  ...
  return (0, import_kolmafia25.visitUrl)(url, method === "POST", !0);
}
...
var Kmail = /* @__PURE__ */ function() {
  ...
  return _createClass2(Kmail2, [{
    ...
    value: function() {
      ...
      return !outsideAttachments && !insideAttachments ? 0 : (0, import_kolmafia26.extractMeat)("".concat(outsideAttachments).concat(insideAttachments));
    }
    /**
     * Reply to kmail
     *
    ...
    value: function() {
      ...
      return JSON.parse((0, import_kolmafia26.visitUrl)("api.php?what=kmail&for=libram&count=".concat(count))).map(Kmail2.parse);
    }
    /**
     * Bulk delete kmails
     *
    ...
    value: function(to, message, items, meat, chunkSize, constructUrl, successString) {
      var m = meat, sendableItems = _toConsumableArray6(arrayToCountedMap(items).entries()).filter(function(_ref3) {
        ...
        return (0, import_kolmafia26.isGiftable)(item);
...
function getExcavatorVersion() {
  return (0, import_kolmafia27.gitInfo)("loathers-excavator-release").commit.substring(0, 7) || (0, import_kolmafia27.gitInfo)("gausie-excavator-release").commit.substring(0, 7) || 0;
}
function getVersionString() {
  return "".concat((0, import_kolmafia27.getRevision)(), "/").concat(getExcavatorVersion());
}
function canKmail() {
  return (
    // In a fight
    !((0, import_kolmafia27.currentRound)() > 0 || // In a choice
    (0, import_kolmafia27.handlingChoice)() || // Was in a choice, gonna be in a fight
    (0, import_kolmafia27.fightFollowsChoice)() || // Was in a fight, gonna be in a choice
    (0, import_kolmafia27.choiceFollowsFight)() || // Was in a fight, gonna be in another fight
    (0, import_kolmafia27.inMultiFight)())
  );
}
...
function flushSpadingData() {
  for (var spadingData = get(DATA_PROPERTY).split("|"), i = 0; i < spadingData.length - 2; ) {
    ...
    if (!success) {
      (0, import_kolmafia27.print)("Sending a kmail failed while Excavator was flushing the spading cache. Flush aborted.", "red");
      return;
    }
    i++;
  }
...
function sendSpadingData(projectName, data) {
  ...
  if (canKmail()) {
    ...
    (0, import_kolmafia27.printHtml)('<font color="green">Sending spading data for <b>'.concat(projectName, "</b>").concat(flushMessage, " to ").concat(recipient, ". Thanks!</font>"));
    var success = Kmail.send(recipient, dataString);
    if (success) {
      flushSpadingData(), deleteSpadingKmail(recipient);
      return;
    }
    (0, import_kolmafia27.print)("Excavator thought it could send data via kmail but it can't. Saving to cache instead.", "orange");
  }
  addSpadingData(dataString, recipient, "Excavator's project to spade ".concat(projectName));
}
function deleteSpadingKmail(sentTo) {
  var buffer = (0, import_kolmafia27.visitUrl)("messages.php?box=Outbox&begin=1&order=0&per_page=10").toLowerCase(), messageIds = buffer.split("td valign=top").filter(function(s) {
  ...
  if (messageIds.length > 0) {
    ...
    (0, import_kolmafia27.visitUrl)(del);
  }
}
...
function main(event, meta, page) {
  projects.filter(function(_ref) {
    ...
    return !completed && event in hooks && since <= (0, import_kolmafia28.getRevision)();
  }).map(function(_ref2) {
